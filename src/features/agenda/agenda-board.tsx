...
function PendingTray({ visible, pendingCount, problemCount, tray, setTray, search, setSearch, grouped, setGrouped, groups }: { visible: AgendaRequest[]; pendingCount: number; problemCount: number; tray: 'pending' | 'problems'; setTray: (value: 'pending' | 'problems') => void; search: string; setSearch: (value: string) => void; grouped: boolean; setGrouped: (value: boolean) => void; groups: [string, AgendaRequest[] | undefined][] }) {
  ...
  return <aside ref={setNodeRef} className={`flex max-h-[690px] flex-col overflow-hidden rounded-2xl border bg-white transition ${isOver ? 'border-[#3e7d59] ring-4 ring-[#dcecdf]' : 'border-[#d7e0da]'}`}>
    ...
    <div className="space-y-2 overflow-y-auto p-3">{grouped ? groups.map(([seller, items]) => <div key={seller}><div className="mb-2 flex items-center justify-between text-[10px] font-extrabold uppercase tracking-wide text-[#73827a]"><span>{seller}</span><span>{items?.length} numbers</span></div>{items?.map((request) => <NumberCard key={request.number || Math.random()} request={request} />)}</div>) : visible.map((request) => <NumberCard key={request.number || `${request.sellerId}-${request.fechaCreacion}`} request={request} />)}{visible.length === 0 && <div className="grid min-h-36 place-items-center rounded-xl border border-dashed border-[#d7e0da] p-4 text-center text-xs text-[#7c8a83]"><span><Check className="mx-auto mb-2" size={20} />No hay registros en esta bandeja</span></div>}</div>
  </aside>;
}
...
function DayColumn({ day, warehouse, requests, capacities }: { day: { date: string; day: string; label: string }; warehouse: WarehouseId; requests: AgendaRequest[]; capacities: Record<string, number> }) {
  ...
  return <section ref={setNodeRef} className={`day-column ${isOver ? 'is-over' : ''} ${status === 'danger' ? 'is-danger' : ''}`}><header className="border-b border-[#e1e8e3] bg-white p-3"><div className="flex items-baseline justify-between gap-2"><h2 className="text-sm font-extrabold capitalize">{day.day} <span className="font-semibold text-[#718078]">{day.label}</span></h2>{metrics.utilization == null ? <span className="status-pill neutral">Sin definir</span> : <span className={`status-pill ${status}`}>{percentageFormat.format(metrics.utilization)}%</span>}</div>{metrics.capacity == null ? <div className="mt-3 rounded-lg bg-[#f2f4f2] px-2 py-2 text-[10px] font-bold text-[#718078]">Capacidad no definida</div> : <><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#e5ece7]"><div className={`capacity-fill ${status}`} style={{ width: `${Math.min(metrics.utilization ?? 0, 100)}%` }} /></div><p className="mt-2 text-[11px] font-bold">{numberFormat.format(metrics.used)} / {numberFormat.format(metrics.capacity)}</p><p className={`text-[10px] ${metrics.available! < 0 ? 'font-bold text-[#b34f2d]' : 'text-[#74837b]'}`}>{metrics.available! < 0 ? `${numberFormat.format(Math.abs(metrics.available!))} sobre capacidad` : `${numberFormat.format(metrics.available!)} disponibles`}</p></>}<div className="mt-2 flex gap-3 text-[10px] text-[#7a8881]"><span>{metrics.assigned.length} numbers</span><span>{metrics.sellers} sellers</span></div></header><div className="space-y-2 p-2">{metrics.assigned.map((request) => <NumberCard key={request.number} request={request} compact />)}<div className="grid min-h-20 place-items-center rounded-xl border border-dashed border-[#ccd8d0] text-center text-[10px] font-semibold text-[#8a9990]"><span><Layers3 className="mx-auto mb-1" size={15} />Soltar aquí</span></div></div></section>;
}
...
function NumberCard({ request, compact = false, overlay = false }: { request: AgendaRequest; compact?: boolean; overlay?: boolean }) {
  ...
  return <article ref={setNodeRef} style={style} {...listeners} {...attributes} className={`number-card ${compact ? 'compact' : ''} ${overlay ? 'overlay' : ''} ${isDragging ? 'opacity-30' : ''} ${request.planningStatus === 'Con problema' ? 'problem' : ''}`}><div className="flex items-start gap-2"><GripVertical className="mt-0.5 shrink-0 text-[#9bad9f]" size={15} /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><p className="truncate text-[11px] font-extrabold tracking-wide">{request.number || 'SIN NUMBER'}</p>{request.priority !== 'Normal' && <span className={`priority ${request.priority.toLowerCase()}`}>{request.priority}</span>}</div>{request.origin === 'provisional' && <p className="mt-1 text-[9px] font-bold text-amber-700">PROVISORIA · editar en Provisorias</p>}{request.origin === 'sheet' && <p className="mt-1 text-[9px] font-bold text-[#4f735f]">FORMULARIO{request.promotedFromProvisional ? ' · reserva conservada' : ''}</p>}<p className="mt-1 truncate text-[11px] text-[#66776e]">{request.sellerName || 'Seller sin nombre'}</p><div className="mt-2 flex items-end justify-between gap-2"><p className="text-sm font-extrabold">{request.unitsMissing ? '—' : numberFormat.format(request.units)} <span className="text-[9px] font-medium text-[#7c8b83]">uds</span></p>{request.validationStatus === 'error' ? <AlertTriangle size={14} className="text-[#b45331]" /> : <p className={`text-[9px] font-bold ${within ? 'text-[#4f735f]' : 'text-[#b45331]'}`}>{request.fechaInicio?.slice(5)} → {request.fechaFin?.slice(5)}</p>}</div>{request.validationMessages.length > 0 && <p className="mt-2 text-[9px] font-semibold text-[#a84f2e]">{request.validationMessages[0]}</p>}</div></div></article>;
}

function Kpi({ label, value, detail, alert = false }: { label: string; value: string; detail: string; alert?: boolean }) {
  return <article className="rounded-xl border border-[#dce5df] bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wide text-[#718078]">{label}</p><p className={`mt-1 text-xl font-extrabold ${alert ? 'text-[#b25431]' : ''}`}>{value}</p><p className="text-[10px] text-[#7b8981]">{detail}</p></article>;
}
